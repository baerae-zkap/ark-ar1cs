...
fn exported_matrices_match_original() {
    ...
    cs1.finalize();
    let matrices_direct = ark_ar1cs::format::ConstraintMatrices::from_cs(&cs1).unwrap();
    ...
    cs2.finalize();
    let matrices_imported = ark_ar1cs::format::ConstraintMatrices::from_cs(&cs2).unwrap();
