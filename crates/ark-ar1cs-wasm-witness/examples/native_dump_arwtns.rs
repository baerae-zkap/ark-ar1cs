...
use ark_ar1cs_wasm_witness::mock::{LargeMockGenerator, EMBEDDED_LARGE_AR1CS_BLAKE3, LargeMockInput};
use std::io::Write;
...
fn main() {
    ...
    let arwtns = witness_generator_native::<LargeMockGenerator>(
        &bytes, &EMBEDDED_LARGE_AR1CS_BLAKE3, &EMBEDDED_LARGE_AR1CS_BLAKE3,
    ).unwrap();
